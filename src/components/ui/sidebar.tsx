...
function useSidebar() {
  ...
  if (!context) {
    // Provide safe defaults when there's no provider so components won't crash.
    // Default to collapsed/closed so the sidebar doesn't appear open by default.
    return {
      state: "collapsed",
      open: false,
      setOpen: () => {},
  ...
  (
    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        ...
        // Do not persist sidebar state to cookies anymore (Option A):
        // previously we wrote `sidebar_state` here which caused the sidebar
        // to persist open/closed across sessions; stop writing to cookies so
        // the UI follows the code default (`defaultOpen`).
      },
      [setOpenProp, open],
    );
  ...
  const { isMobile, state, openMobile, setOpenMobile, open, setOpen } = useSidebar();
    ...
    return (
      <div
        ...
        data-side={side}
      >
        {/* This spacer previously reserved sidebar width and pushed layout.
            Use w-0 when collapsed/offcanvas so the sidebar overlays instead. */}
        <div
          className={cn(
            "relative bg-transparent transition-[width] duration-200 ease-linear",
            // Always keep spacer width at zero so sidebar overlays and doesn't push layout.
            "w-0",
            "group-data-[side=right]:rotate-180",
            className,
          )}
        />
        {/* Overlay to dim the rest of the site when the sidebar is open on desktop */}
        {/* Overlay: always mounted so we can animate opacity for fade in/out */}
        <div
          onClick={() => setOpen(false)}
          aria-hidden
          className={cn(
            "fixed inset-0 z-9 hidden md:block",
            "transition-opacity duration-50 ease-linear",
  ...
  ({ className, ...props }, ref) => {
    return (
      <main
        ...
        className={cn(
          // Base layout
          "relative flex w-full flex-1 flex-col bg-background overflow-y-scroll md:overflow-auto md:transition-none",
          // When the sibling Sidebar (peer) is expanded on md+, reserve the full width.
          // When it's collapsed (icon mode), reserve only the icon width.
